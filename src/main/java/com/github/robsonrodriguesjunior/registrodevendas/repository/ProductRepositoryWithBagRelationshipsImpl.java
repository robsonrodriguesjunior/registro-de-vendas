...
public class ProductRepositoryWithBagRelationshipsImpl implements ProductRepositoryWithBagRelationships {
    ...
    public Optional<Product> fetchBagRelationships(Optional<Product> product) {
        return product.map(this::fetchSales);
    }
    ...
    public List<Product> fetchBagRelationships(List<Product> products) {
        return Optional.of(products).map(this::fetchSales).orElse(Collections.emptyList());
    }

    Product fetchSales(Product result) {
        return entityManager
            .createQuery("select product from Product product left join fetch product.sales where product.id = :id", Product.class)
            .setParameter("id", result.getId())
            .getSingleResult();
    }

    List<Product> fetchSales(List<Product> products) {
        ...
        List<Product> result = entityManager
            .createQuery("select product from Product product left join fetch product.sales where product in :products", Product.class)
